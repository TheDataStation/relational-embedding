...
def display_pca_scatterplot(path_to_model, task, words=None):
    ...
    if "word2vec" in path_to_model:
        twodim["type"] = pd.Series(words).apply(
            lambda x: convert_code_to_three_categories(x))
    if "node2vec" in path_to_model:
        twodim["type"] = pd.Series(words).apply(
            lambda x: "row&col" if "row" in x or "col" in x else convert_code_to_three_categories(
                x)
        )
    ...
    plt.savefig("./embedding_plots/" + task + "/" +
                model_name.split(".")[0] + ".png")
...
if __name__ == "__main__":
    ...
    # EU.all_files_in_path(word2vec_embedding_storage, task)
    all_w2v_emb_path = []
