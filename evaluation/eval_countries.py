...
target_df = pd.read_csv(data_path + 'base_processed.csv')
df_joined = target_df.merge(data_df, how='inner', on='Country Code')

df_joined.drop(columns=['Country Name', 'Indicator Name'], inplace=True)
df_joined = df_joined.drop('Country Code', axis=1).join(pd.get_dummies(df_joined['Country Code']))
df_joined = df_joined.drop('Indicator Code', axis=1).join(pd.get_dummies(df_joined['Indicator Code']))

def simple_regression(X_train, X_test, y_train, y_test):
    X_train = X_train.drop(["2012"], axis = 1)
    X_test = X_test.drop(["2012"], axis = 1)
    lr = LinearRegression()
...
if __name__ == "__main__":
    print("Loading & splitting Countries data")
    df = pd.read_csv(os.path.join(data_path, "base_processed.csv"))
    df = df.drop('Country Code', axis=1).join(pd.get_dummies(df['Country Code']))
    df = df.fillna(0)

    #df = EU.quantize(df, excluding = ['2012'])
    df_joined = df_joined.fillna(0)

    X = df
    Y = df['2012'].values.reshape(-1, 1)
    X_train, X_test, y_train, y_test = train_test_split(X,Y, test_size = test_size, random_state=10)
    
    X_joined = df_joined
    Y_joined = df_joined['2012'].values.reshape(-1, 1)
    ...
    # Baseline 4: Join all tables & Random Forest
    print("Baseline 4: Joined & Random Forest")
